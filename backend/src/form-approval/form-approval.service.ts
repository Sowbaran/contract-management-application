...
export class FormApprovalService implements OnModuleInit {
    ...
    if (formDetailsModels) {
      ...
      if (status !== FormStatus.REJECTED) {
        ...
        if (nextWorkflow) {
          ...
          // DOCUSIGN CODE COMMENTED OUT - Workflow continues without DocuSign
          ...
          // WORKFLOW WITHOUT DOCUSIGN: When eSignRequired is true, treat it same as false - set status to COMPLETED
          if (updateFormApprovalDto.eSignRequired) {
            this.logger.log(`Contract Form Approval - eSign required but DocuSign disabled, proceeding to COMPLETED status`);
          }
          
          // Use the same logic as when eSignRequired is false
          {
  ...
  // DOCUSIGN CALLBACK METHOD COMMENTED OUT - Not needed without DocuSign
  ...
  // DOCUSIGN RETRIGGER METHOD COMMENTED OUT - Not needed without DocuSign
