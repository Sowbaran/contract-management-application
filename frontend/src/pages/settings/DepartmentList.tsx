...
import { useGetAllDepartmentsAdmin } from "../../api/backend/backendComponents";
import { DepartmentForm } from "./DepartmentForm";
import { useQueryClient } from "@tanstack/react-query";

type SortKeys = "departmentName" | "description";
...
export const DepartmentList = () => {
  ...
  const { data } = useGetAllDepartmentsAdmin(
    {
      headers: {
  ...
  useEffect(() => {
    if (!drawerOpen) {
      console.log(`departList drawerOpen:${drawerOpen}`);
      queryClient.invalidateQueries();
    }
  }, [drawerOpen, queryClient]);

  useEffect(() => {
    const filteredData = alteredTableRes.filter((department: AnyProp) => {
      return department.name.toLowerCase().includes(searchInput.toLowerCase());
    });
